...
public class EventsDao {
	...
	public List<Integer> getAnni(){
		String sql="select distinct year(reported_date) as days "
				+ "from EVENTS "
				+ "order by days";
	...
	public List<Adiacenza> getAdiacenze(String categoria, int giorno){
		String sql="select e1.offense_type_id as id1, e2.`offense_type_id` as id2, count(distinct e1.district_id) as peso "
				...
				+ "where e1.offense_category_id=? and e1.`offense_category_id`= e2.`offense_category_id` and e1.district_id=e2.district_id and year(e1.reported_date)=? and e1.`offense_type_id`<e2.`offense_type_id` and year(e2.`reported_date`)=? "
				+ "group by id1, id2";
